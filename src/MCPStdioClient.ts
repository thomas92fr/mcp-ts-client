 ...
 * @property command - Commande à exécuter (ex: 'node', 'npx')
 * @property args - Arguments de la commande
 * @property env - Variables d'environnement optionnelles
 */
 ...
 * @property mcpServers - Map des configurations de serveurs indexée par nom
 */
...
/**
 * Client pour communiquer avec un serveur MCP via stdio.
 * Gère les sessions JSON-RPC, l'invocation d'outils et la gestion des erreurs.
 */
export class MCPStdioClient {
   ...
   */
  constructor(serverConfig: ServerConfig, logger?: ILogger) {
    this.logger = logger;
  ...
  async listTools<T = any>(parameters: Record<string, any> = {}): Promise<T> {
    if (!this.sessionId) {
      throw new Error(
        "No active session. Make sure to wait for session initialization."
      );
    }
  ...
  async invokeTool<T = any>(
    ...
    return new Promise((resolve, reject) => {
      ...
      try {
        this.logger?.debug(
          "Sending tool invocation request:",
          JSON.stringify(request, null, 2)
        );
        this.process.stdin.write(JSON.stringify(request) + "\n");
      } catch (error) {
  ...
  async readMultipleFiles(paths: string[]): Promise<Record<string, string>> {
    return this.invokeTool<Record<string, string>>("read_multiple_files", {
      paths,
    });
  }
...
/**
 * Crée les clients MCP à partir d'un fichier de configuration.
 * Ne retourne que les clients qui ont réussi leur initialisation.
 *
 * @param configPath - Chemin vers le fichier de configuration JSON
 * @param logger - Logger optionnel pour tracer les erreurs
 * @returns Liste des clients MCP initialisés avec succès
 * @throws Error si le fichier de configuration est invalide ou inaccessible
 */
export async function createMCPClientsFromConfig(
  configPath: string,
  logger?: ILogger
): Promise<MCPStdioClient[]> {
  const configContent = await fs.readFile(configPath, "utf8");
  const config: ConfigFile = JSON.parse(configContent);
  const initializedClients: MCPStdioClient[] = [];

  for (const [name, serverConfig] of Object.entries(config.mcpServers)) {
    try {
      // Pour les commandes PATH-based comme npx, ne pas vérifier le chemin
      const client = new MCPStdioClient(serverConfig, logger);
      try {
